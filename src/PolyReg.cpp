...
void PolyReg::fit(PnlVect* stock_values, PnlVect* discounted_CF){
    ...
    switch( choix_type(poly_type_) )
    {
            ...
            for(int i = 0; i < n_obs; i++){
                ...
                if(stock_value > 0){
                    for(int degree = 0; degree <= max_degree_; degree++){
                        *(xy + counter * n_degree + degree) = chebyshevcalculate(1, degree, stock_value);

                    }
                    *(xy + counter * n_degree + max_degree_ + 1) = GET(discounted_CF, i);
                    counter++;
                }
        ...
        case Chebyshev2: {
            for(int i = 0; i < n_obs; i++){
                ...
                if(stock_value > 0){
                    for(int degree = 0; degree <= max_degree_; degree++){
                        *(xy + counter * n_degree + degree) = chebyshevcalculate(2, degree, stock_value);
                    }
                    *(xy + counter * n_degree + max_degree_ + 1) = GET(discounted_CF, i);
                    //cout<<xy[i][0]<<" | "<<xy[i][1]<<" | "<<xy[i][2]<<" | "<<xy[i][3]<<endl;
                    counter++;
        ...
        case Hermite: {
            for(int i = 0; i < n_obs; i++){
                ...
                if(stock_value > 0){
                    for(int degree = 0; degree <= max_degree_; degree++){
                        *(xy + counter * n_degree + degree) = hermitecalculate( degree, stock_value);
                    }
                    *(xy + counter * n_degree + max_degree_ + 1) = GET(discounted_CF, i);
                    //cout<<xy[i][0]<<" | "<<xy[i][1]<<" | "<<xy[i][2]<<" | "<<xy[i][3]<<endl;
                    counter++;
        ...
        case Legendre: {
            for(int i = 0; i < n_obs; i++) {
                ...
                if (stock_value > 0) {
                    for (int degree = 0; degree <= max_degree_; degree++) {
                        *(xy + counter * n_degree + degree) = legendrecalculate(degree, stock_value);
                    }
                    *(xy + counter * n_degree + max_degree_ + 1) = GET(discounted_CF, i);
                    //cout<<xy[i][0]<<" | "<<xy[i][1]<<" | "<<xy[i][2]<<" | "<<xy[i][3]<<endl;
                    counter++;
        ...
        case Laguerre: {
            for(int i = 0; i < n_obs; i++){
                ...
                if (stock_value > 0) {
                    for (int degree = 0; degree <= max_degree_; degree++) {
                        *(xy + counter * n_degree + degree) = laguerrecalculate(degree, stock_value);
                    }
                    *(xy + counter * n_degree + max_degree_ + 1) = GET(discounted_CF, i);
                    //cout<<xy[i][0]<<" | "<<xy[i][1]<<" | "<<xy[i][2]<<" | "<<xy[i][3]<<endl;
                    counter++;
    ...
    xy_2d_array.attach_to_ptr(counter,max_degree_+2,xy);
...
void PolyReg::fit2(PnlVect *stock_values1, PnlVect *stock_values2 , PnlVect *discounted_CF) {
    ...
    int max=(max_degree_+1) * (max_degree_+1);
    int maxp = max + 1;
    double* xy = new double[n_obs * maxp];
    double stock_value1;
    double stock_value2;
    int counter = 0;
    switch (choix_type(poly_type_)) {
            ...
            for (int i = 0; i < n_obs; i++) {
                ...
                if (stock_value1 * stock_value2 > 0) {
                    for (int degree = 0; degree < max; degree++) {
                        *(xy + counter * maxp + degree) = chebyshevcalculate(1, degree % (max_degree_ + 1), stock_value1) *
                                              chebyshevcalculate(1, degree / (max_degree_ + 1), stock_value2);

                    }
                    *(xy + counter * maxp + max) = GET(discounted_CF, i);
        ...
        case Chebyshev2: {
            for (int i = 0; i < n_obs; i++) {
                ...
                if (stock_value1 * stock_value2 > 0) {
                    for (int degree = 0; degree < max; degree++) {
                        *(xy + counter * maxp + degree) = chebyshevcalculate(2, degree % (max_degree_ + 1), stock_value1) *
        ...
        case Hermite: {
            for (int i = 0; i < n_obs; i++) {
                ...
                if (stock_value1 * stock_value2 > 0) {
                    for (int degree = 0; degree < max; degree++) {
                        *(xy + counter * maxp + degree) = hermitecalculate(degree % (max_degree_ + 1), stock_value1) *
        ...
        case Legendre: {
            for (int i = 0; i < n_obs; i++) {
                ...
                if (stock_value1 * stock_value2 > 0) {
                    for (int degree = 0; degree < max; degree++) {
                        *(xy + counter * maxp + degree) = legendrecalculate(degree % (max_degree_ + 1), stock_value1) *
        ...
        case Laguerre: {
            for (int i = 0; i < n_obs; i++) {
                ...
                if (stock_value1 * stock_value2 > 0) {
                    for (int degree = 0; degree < max; degree++) {
                        *(xy + counter * maxp + degree) = laguerrecalculate(degree % (max_degree_ + 1), stock_value1) *
    ...
    real_2d_array xy_2d_array;
    xy_2d_array.attach_to_ptr(counter, max + 1, xy);
    //printf("%s\n", xy_2d_array.tostring(4).c_str());
